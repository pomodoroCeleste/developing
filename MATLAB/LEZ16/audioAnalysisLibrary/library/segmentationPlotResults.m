...
totalDuration = length(a) / fs;    % get the input file's duration in seconds
...
while (1)
    ...
    [a, fs] = audioread(fileName); a=length(a);
    [xselected, fs] = audioread(fileName, round([segs(Iselected, 1) * fs + 1 segs(Iselected, 2) * fs]));
